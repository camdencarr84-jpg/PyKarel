# Torbert, 8.25.2005
# Grover, modified 2021 & 2023

from tkinter import *                  #Grover --- Changed from 'TKinter' because of pyton 3.9
...
class Robot:
    COUNT = 1
    def __init__(self, world, x=1, y=1, direction=east, beepers=0, picName = "karel", fileName = "karel"):
        self.world, self.image = world, None
        self.x, self.y, self.d, self.beepers = x, y, direction%360, beepers
    #------ Grover --- add custom images -------------------------------------------------
        self.picName = picName
        self.fileName = fileName
        self.setImages(self.picName,  self.fileName)
    #----------------------------------- -------------------------------------------------
        self.alive = True
...
#-----------Grover Additions------------------------------------------------------------
    ...
    def teleport(self, newX, newY):
        self.world.recordMove(self.count, self.x, self.y, newX, newY)
        self.x = newX
        self.y = newY
        self.draw()
        self.world.refresh()
    ...
    def avenue(self):       #get Methods not used in python  
        return self.x
         
    def street(self):       #get Methods not used in python  
        return self.y

    def setImages(self, pName, fName):
        self.photos =  [PhotoImage(file=("pics/" + pName + "/" + fName + k + ".gif")) for k in ["e","n","w","s"]]  
...
#---------------------------------------------------------------------------------------
    ...
    def __str__(self):
        ...
        return "Robot %d at (%d,%d) facing %s carrying %s beeper(s)." % \
               (self.count, self.x, self.y, ["east", "north", "west", "south"][int(self.d/90)], s)   #Grover --- self.d/90 had to be converted to int
    ...
    def draw(self):
        if not self.alive:
            raise Exception("Robot has been destroyed.")
        self.image = self.world.draw(self.x, self.y, self.d, self.image, self.photos)      #Grover --- needed to send custom photos list for custom images
    ...
    def onABeeper(self):    #Grover --- changed to onABeeper, nextToABeeper was confusing
        if not self.alive:
            raise Exception("Robot has been destroyed.")
        return self.world.isBeeper(self.x, self.y)

    def onARobot(self):     #Grover --- changed to onARobot, nextToARobot was confusing
        if not self.alive:
            raise Exception("Robot has been destroyed.")
...
class World(Tk):
    ...
    #------ Grover --- removed photos array from the World class & put in Robot class to be able to have custom images
        ...
        for k in range(2*self.t, max(self.m,self.n)-self.t, self.t):
            if k < b: 
                ...
                self.canvas.create_text(k, c+self.t/2, text=str(count), font=("Times",max(int(-self.t*2/3),-15),""))                #Grover --- had to convert '-self.t*2/3' in int for python 3.9 to be able to shrink world
            if k < c: 
                self.canvas.create_line(b, k, a, k, fill="red")
                self.canvas.create_text(a-self.t/2, self.n-k, text=str(count), font=("Times",max(int(-self.t*2/3),-15),""))          #Grover --- had to convert '-self.t*2/3' in int for python 3.9 to be able to shrink world 
            count += 1
    ...
    def addInfiniteBeepers(self,x,y):
        flag = (x,y) not in self.beepers.keys() or self.beepers[(x,y)] == 0        
    ...
    def addBeeper(self,x,y):
        ...
        a, b = self.t+x*self.t, self.n-(self.t+y*self.t)
        t = self.t/4        #Grover --- made smaller beepers so custom images would cover (was t/3)
        if flag:
            self.ovals[(x,y)] = self.canvas.create_oval(a-t, b-t, a+t, b+t, fill="black")
            self.numbers[(x,y)] = self.canvas.create_text(a, b, text=text, fill="white", font=("Times",max(int(-self.t/2),-20),""))   #Grover --- had to convert '-self.t/2' in int for python 3.9
        else:
            self.canvas.itemconfig(self.numbers[(x,y)], text=text)
        if (x,y) in self.robots.keys():
            for robot in self.robots[(x,y)]:
    ...
    def draw(self,x,y,d,img, photos):         #Grover --- add parameter to send the Robot's image to be drawn
        if self.useImage:
            if img is not None:
                self.canvas.delete(img)
            x, y = self.t+x*self.t, self.n-(self.t+y*self.t)
            photo = photos[int(d/90)]         #Grover --- d/90 had to be converted to int for python 3.9
                                              #Grover --- and didn't use the World's images
            img = self.canvas.create_image(x, y, image=photo)       #Grover --- use the Robt's image instead     
            return img
        else:
